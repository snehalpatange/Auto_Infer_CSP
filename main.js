...
// Create a page mod it will run a script whenever a URL is loaded
pageMod.PageMod({
  include: ["*","file://*"],
  contentScriptWhen: "end",
  contentScriptFile: self.data.url("extraction-script.js"),
  // Send the content script a message inside onAttach to start extracting the script tags from the loaded URL content
  onAttach: function(worker) {
	worker.port.on('saveScript', function(args) {
      saveScript(args);
    });
	worker.port.emit("extractContent");
	var scripts_folder = 'C:\\extracted_scripts\\';
	var ur = decodeURI(tabs.activeTab.url);
	var p = ur.replace(/(\?.*)|(#.*)/g, "");
	console.log("-------------------------------------------" + p);
	var fileName = p.replace(/[\/:*"<>|?]/g, '_');
	var theFile = scripts_folder + fileName + ".js";
	worker.port.emit("replaceJavaScript", theFile);
  }
});
...
function saveScript(args){
	...
	var fileName = p.replace(/[\/:*"<>|?]/g, '_');
	var theFile = scripts_folder + fileName + ".js";
	console.log("Saving script to " + theFile);
	...
	if(file.exists() == false) //check to see if file exists
	{
		...
		console.log("Script file already exists " + ". EXITING...");
		return "";
	}
...
userCSP.port.on("reloadPage", function (domain) {
    for (var i = 0; i < tabs.length; i++) {
        var tempStr = URL(tabs[i].url).scheme + "://" + URL(tabs[i].url).host;
        if (tempStr === domain) {
			tabs[i].reload();
		}
	}
